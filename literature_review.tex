...
\begin{titlepage}
    ...
    {\Large \color{sectioncolor} A Hybrid Sentiment-Aware Stock Prediction Study} \\[0.4cm]
...
The stock market is a core component of modern financial systems and a major channel for capital allocation. Predicting stock prices is difficult because market behavior is non-linear, noisy, and regime-dependent. Price dynamics reflect many interacting drivers, including company performance, macroeconomic conditions, global events, liquidity, and investor behavior.

Traditional statistical models provide interpretability and strong theoretical grounding, but often struggle under rapid market shifts and complex non-linear interactions. Machine learning and deep learning methods improved pattern discovery, yet many price-only systems still miss important exogenous signals.
...
\begin{itemize}
    ...
    \item Critically compare statistical, machine learning, deep learning, and hybrid systems using stock-domain evidence.
...
\begin{enumerate}
    \item \textbf{Critical Stock-Domain Synthesis.} The literature review is organized around stock-market evidence, with explicit comparison by model family, evaluation protocol, and deployment limitations.
    \item \textbf{Hybrid Forecasting Framework.} The methodology defines a residual-learning hybrid pipeline that combines SARIMA trend modeling with XGBoost non-linear correction.
...
Early stock-forecasting studies relied on linear time-series modeling. ARIMA-style modeling remains a standard baseline because of interpretability and transparent diagnostics \cite{box2015}. Volatility-focused modeling evolved through ARCH and GARCH, which are still foundational in risk and variance forecasting for financial series \cite{engle1982,bollerslev1986}. These methods are valuable as reference models, but their assumptions limit performance under abrupt structural breaks and strong non-linearity.

\subsubsection{Transition to Machine Learning}
Machine learning introduced non-linear decision boundaries and feature interactions beyond linear econometric assumptions. SVM-based forecasting \cite{kim2003} and later ensemble/tree methods in equity-index settings \cite{patel2015,nti2020review} showed that technical features and robust preprocessing can outperform basic linear baselines in direction classification tasks. However, performance varies by market regime, horizon, and feature construction, making reproducibility and protocol design central concerns.

\subsubsection{Deep Learning Era}
Sequence models such as LSTM became prominent because they model temporal dependencies without fully manual lag engineering \cite{hochreiter1997}. In stock-domain evaluations, LSTM-class models frequently improve directional performance relative to shallow baselines, especially when feature windows are tuned and leakage is controlled \cite{fischer2018,sezer2020}. At the same time, gains are not uniform across assets: data quality, horizon choice, and market regime materially affect results.

\subsubsection{From Sequence Models to Attention and Multimodal Systems}
Attention-based architectures, motivated by transformer modeling \cite{vaswani2017}, enabled richer long-context learning and multimodal fusion strategies. In financial prediction, event-driven text-price modeling demonstrated measurable improvements when textual events were aligned with market movements \cite{ding2015}. This shift established a key design principle: financial forecasting quality depends on both temporal modeling and information-source alignment.

\subsection{Existing Systems / Related Work}

\subsubsection{Traditional Statistical Methods}
ARIMA/SARIMA remain strong baselines for trend and seasonality decomposition when assumptions are approximately satisfied \cite{box2015}. Their strengths are interpretability, low compute cost, and stable diagnostics. Their limitations are well-known in equity forecasting: weak handling of non-linear cross-feature effects, sensitivity to structural breaks, and reduced adaptability under rapid regime transition.

\subsubsection{Machine Learning Approaches}
SVM, random forests, and gradient-boosting systems are widely used for stock direction prediction and residual correction tasks \cite{kim2003,patel2015,nti2020review}. Across studies, model quality is heavily conditioned on feature design (lags, technical indicators, macro context) and temporal validation protocol. A recurring finding is that strong preprocessing and feature engineering can narrow or exceed the gap to more complex deep models in certain horizons.

\subsubsection{Deep Learning-Based Approaches}
LSTM/GRU/BiLSTM families are the most common deep architectures in financial forecasting benchmarks \cite{sezer2020}. In stock-specific tests, LSTM-based systems often provide better sequence modeling than static learners, especially for short-horizon direction tasks \cite{fischer2018}. Still, deep models can overfit in non-stationary markets and require careful walk-forward evaluation, retraining policy, and latency-aware deployment constraints.

\subsubsection{Sentiment-Based Methods}
Sentiment modeling evolved from dictionary approaches (e.g., domain lexicons) to transformer-based contextual models. Financial-domain language resources \cite{loughran2011,malo2014} and social-media market-signal studies \cite{bollen2011} established that textual information can carry predictive value under specific settings. Domain-adapted transformers such as FinBERT \cite{araci2019}, built on BERT \cite{devlin2019}, improved finance-text polarity classification; however, production use still depends on timestamp alignment, confidence handling, and drift control.

\subsection{Hybrid Architectures}

\subsubsection{Rationale for Hybrid Design}
...
\subsubsection{Price-Only Hybrid Systems}
ARIMA/SARIMA plus tree-boosting or neural residual correction is a common design pattern in stock prediction studies and industrial prototypes. Survey evidence indicates that hybrid and ensemble families are frequently more robust than single-model pipelines under heterogeneous market conditions \cite{nti2020review,sezer2020}. Practical success depends on leakage-safe residual construction and consistent out-of-sample testing.

\subsubsection{Price + Sentiment Hybrid Systems}
Recent systems extend hybrid pipelines with sentiment features from financial news and event streams. Event-driven deep models \cite{ding2015} and sentiment-market evidence \cite{bollen2011} motivate this integration, while financial-domain NLP resources \cite{loughran2011,araci2019} improve feature quality. The key unresolved engineering issue is not only sentiment scoring accuracy, but robust alignment between asynchronous text events and tradable market time.

\subsection{Comparative Evidence from Literature}

\begin{table}[htbp]
\centering
\caption{Representative Evidence in Stock-Market Forecasting Literature}
\label{tab:literature_matrix}
\small
\begin{tabular}{@{}p{3cm}p{2.2cm}p{2.6cm}p{2.5cm}p{3cm}@{}}
\toprule
\textbf{Study} & \textbf{Market / Data} & \textbf{Method Family} & \textbf{Typical Metrics} & \textbf{Main Limitation} \\ \midrule
Kim (2003) \cite{kim2003} & Equity index direction data & SVM classifier & Directional accuracy & Sensitive to feature specification and regime shifts \\
\addlinespace
Patel et al. (2015) \cite{patel2015} & Indian equities + indices & ML comparison (ANN/SVM/RF/NB) & Accuracy, precision/recall & Preprocessing-dependent gains \\
\addlinespace
Fischer \& Krauss (2018) \cite{fischer2018} & S\&P 500 constituents & LSTM vs classical baselines & Accuracy, return simulation & Stability across regimes remains challenging \\
\addlinespace
Sezer et al. (2020) \cite{sezer2020} & Multi-study review (2005--2019) & Deep-learning survey & Cross-study comparison & Strong protocol heterogeneity \\
\addlinespace
Nti et al. (2020) \cite{nti2020review} & Cross-market stock studies & Ensemble evaluation & Accuracy-based measures & Limited risk-aware reporting \\
\addlinespace
Ding et al. (2015) \cite{ding2015} & S\&P 500 + event text & Event-driven deep model & Direction + simulation uplift & Event extraction and alignment complexity \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:literature_matrix} highlights two consistent observations. First, reported gains are highly protocol-sensitive (split strategy, feature engineering, horizon definition, and market choice). Second, literature still under-reports risk-adjusted and deployment-relevant outcomes compared with raw accuracy metrics.

\subsection{Limitations of Existing Systems}

\subsubsection{General Modeling Limitations}
...
\begin{itemize}
    \item \textbf{Temporal mismatch}: text arrives asynchronously, while price labels are market-clock dependent.
    \item \textbf{Domain drift}: financial language, ticker conventions, and event vocabulary evolve rapidly.
    \item \textbf{Confidence blindness}: many pipelines use polarity scores without reliability weighting.
    \item \textbf{Aggregation mismatch}: document-level polarity may hide aspect-level conflict (e.g., positive revenue, negative guidance).
    \item \textbf{Evaluation mismatch}: many studies optimize RMSE/MAE but under-report trading-risk metrics.
\end{itemize}
